...
--
-- Host: 127.0.0.1
-- Generation Time: May 28, 2021 at 07:47 AM
...
('1234', 'admin_kaloy@gmail.com'),
('access1234', 'access_admin@gmail.com');
...
(24, 'ASUS VZ249HEG1R Gaming Monitor', 'Is this high-difinition?', 'access_admin@gmail.com');
...
('access_admin@gmail.com', 'ADMIN', 'ACCESS', '09998901910', 'dalaguete'),
...
INSERT INTO `transaction_tbl` (`Transaction_id`, `GrandTotal`, `Subtotal`, `emailAddress`, `ProductName`, `Status`) VALUES
(33, '3999', '3999', 'access_admin@gmail.com', 'Hyper X DDR4 Ram 8gb', 'Pending');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_transactions`  AS SELECT `product_tbl`.`Prod_Img` AS `Prod_Img`, `transaction_tbl`.`ProductName` AS `ProductName`, `product_tbl`.`Price` AS `Price`, `transaction_tbl`.`Subtotal` AS `Subtotal`, `transaction_tbl`.`GrandTotal` AS `GrandTotal`, `transaction_tbl`.`emailAddress` AS `emailAddress`, `transaction_tbl`.`Status` AS `Status` FROM (`transaction_tbl` join `product_tbl` on(`product_tbl`.`prodname` = `transaction_tbl`.`ProductName`)) ;
